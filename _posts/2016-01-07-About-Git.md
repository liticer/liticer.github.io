...
&ensp;&ensp;&ensp;&ensp;Git管理下的有四个区，分别为：Workspace(工作区)、Index/Stage(暂存区)、Repository(本地仓库）、Remote(远程仓库)。
...
&ensp;&ensp;&ensp;&ensp;Git的设置文件为.gitconfig，它可以在用户主目录下（全局配置），也可以在项目目录下（项目配置）。
<pre class="prettyPrint lang=bash">
# 显示当前的Git配置
$ git config --list
